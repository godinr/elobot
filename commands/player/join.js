        ...
        try {
            ...
            UserSchema.findOne({id: userId}).then(async (res) => {
                ...
                if (interaction.member.permissions.has('ManageNicknames')){
                    ...
                    joinEmbed.setDescription('Unable to change your nickname due to your permission level.')
                }else {
                    await interaction.member.setNickname(`[${user.rating} ELO] ${username}`);
                    joinEmbed.setDescription(`Your nickname has been changed to [${user.rating} ELO] ${username}\nNew server roles were applied`);
                }
            ...
            console.log('[CMD - Join] | Error catched');
            console.log(err);
        }
